...
Napi::Object Util_exports(Napi::Env env, Napi::Object exports) {
    exports.Set("enablePatch", Napi::Function::New(env, [](const Napi::CallbackInfo& info) {
        bool ok = IUtil->enablePatch(UICommon_GameFile::unwrap(info[0])->iface, asStrUtf8(info[1]).c_str());
        return fromBool(info.Env(), ok);
    }));
    exports.Set("enableARCode", Napi::Function::New(env, [](const Napi::CallbackInfo& info) {
        bool ok = IUtil->enableARCode(UICommon_GameFile::unwrap(info[0])->iface, asStrUtf8(info[1]).c_str());
        return fromBool(info.Env(), ok);
    }));
    exports.Set("enableGeckoCode", Napi::Function::New(env, [](const Napi::CallbackInfo& info) {
        bool ok = IUtil->enableGeckoCode(UICommon_GameFile::unwrap(info[0])->iface, asStrUtf8(info[1]).c_str());
        return fromBool(info.Env(), ok);
    }));
